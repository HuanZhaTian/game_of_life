...
def main():
    ...
    check_var = IntVar()
    check = Checkbutton(top_frame,text="是否随机",variable=check_var)
    check.pack(side="left")

    def callback():

        switch = check_var.get()
        ...
        def get_user_hit(event):
            x, y = event.x / cell_size, event.y / cell_size

            world[x][y] = not world[y][x]
        ...
        if switch:
            ...
            q=1
            while q:
                can.bind("<Button-1>",get_user_hit)
                print_world(world, world_size, can)
                root.update_idletasks()
                root.update()
